...
// $Log: fsastar.cpp $
// Revision 1.7  2010/06/10 13:57:08Z  Alun
// All tabs removed again
// Revision 1.6  2010/05/11 07:39:43Z  Alun
// help changed
// Revision 1.5  2009/10/07 16:26:32Z  Alun
...
/* Program to calculate the not of an FSA. All the real
   work is done by FSA_Factory::star() in fsa.cpp */
#include <string.h>
#include "fsa.h"
#include "container.h"
#include "maf_so.h"
...
#define cprintf container.error_output
  ...
  {
    cprintf("Usage: fsastar [loglevel] [format] -i | input_file [-o | output_file]\n"
            "where either stdin (if you specify -i), or input_file"
            " (otherwise), contains a\nGASP FSA.\n"
            "The output FSA accepts a word if and only if is a concatenation"
            " of an arbitrary\nnumber of words accepted by the input FSA\n");
    so.usage(".star");
  }
  delete &container;
  return exit_code;
                 ...
                 bool use_stdout,unsigned fsa_format_flags)
{
  ...
  if (fsa1)
  {
    fsa2 = FSA_Factory::star(*fsa1);
    if (fsa2)
    {
      String_Buffer sb;
      String var_name = sb.make_filename("",fsa1->get_name(),"_star",false,
                                         String::MFSF_Replace_No_Dot);
      fsa2->set_name(var_name);
      if (!use_stdout)
        filename2 = sb.make_destination(filename2,filename1, ".star");
