...
// $Log: fsanot.cpp $
// Revision 1.6  2010/05/11 07:54:54Z  Alun
// help changed
// Revision 1.5  2009/10/07 16:28:18Z  Alun
// Compatibility of FSA utilities with KBMAG improved.
// Compatibility programs produce fewer FSA KBMAG would not (if any)
// Revision 1.4  2009/09/12 18:47:23Z  Alun
...
/* Program to calculate the not of an FSA. All the real
   work is done by FSA_Factory::fsa_not() in fsa.cpp */
#include "fsa.h"
#include "container.h"
#include "maf_so.h"
...
int main(int argc,char ** argv);
  static int inner(Container * container,String filename1,String filename2,
                   bool first, bool use_stdout,unsigned fsa_format_flags);
...
#define cprintf container.error_output
  ...
  if (!bad_usage && ((input_file!=0) ^ so.use_stdin) &&
    ...
    exit_code = inner(&container,input_file,output_file,first,so.use_stdout,
                      so.fsa_format_flags);
  else
  {
    cprintf("Usage: fsanot [loglevel] [format] -i | input_file [-o | output_file]\n"
            "where either stdin (if you specify -i), or input_file"
            " (otherwise), contains a\nGASP FSA.\n"
            " The output FSA accepts words if and only if the original does not\n");
    so.usage(".not or .firstnot");
  }
  delete &container;
  return exit_code;
}
                 ...
                 bool first,bool use_stdout,unsigned fsa_format_flags)
{
  ...
  if (fsa1)
  {
    fsa2 = FSA_Factory::fsa_not(*fsa1,first);
    if (fsa2)
    {
      String_Buffer sb;
      String var_name = sb.make_filename("",fsa1->get_name(),"_not",false,
