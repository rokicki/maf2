...
// Help changed. Support for SO_STDOUT removed as not needed
// Revision 1.4  2009/10/07 16:47:50Z  Alun
...
#define cprintf container.error_output
  ...
  {
    cprintf("Usage: fsaandnot [loglevel] [format] [-first] file1 file2 [file3]\n"
            "where file1 and file2 contain GASP FSA with the"
            " same alphabet.\n"
            "The output FSA accepts a word if the first FSA accepts it but"
            " the\n second does not. With -first extensions of accepted words"
            " are rejected.\n"
                 ...
                 String filename3,bool first,unsigned fsa_format_flags)
{
  ...
  FSA * answer = first ? FSA_Factory::and_not_first(*fsa1,*fsa2) :
                         FSA_Factory::and_not(*fsa1,*fsa2);
  String_Buffer sb;
